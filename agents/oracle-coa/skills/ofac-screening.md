...
# OFAC (Office of Foreign Assets Control) Screening: Sanctions Verification
...
**Demo runtime:** the COA (Compliance Officer Agent) downloads the public
OFAC SDN (Specially Designated Nationals) dump (ETH/EVM addresses), screens
the subject on every evaluation / Opinion, and on a direct match writes
`SanctionRegistry.setSanctioned`. Layer 1 at swap time is still the mapping.
`beforeSwap` does not call Treasury. Wallet A is **not** on SDN; the agent
writes score 100 from the exploit finding (`WalletBlocked`). A live SDN
exact-address match is hook Layer 1: registry write and `SanctionHit`, not
`WalletBlocked`, and not a use-case wallet. A listed subject, P2P
(peer-to-peer) counterparty, or listed contract the wallet used in the pool
is score 100. Do not hop-contaminate B/C/D from a listed address and do not
fund E from one.
...
| List | Issuer | Scope |
|---|---|---|
| SDN List | OFAC: U.S. Treasury | Broad: USD (United States dollar), U.S. nexus, U.S. persons |
| Non-SDN sectoral (SSI, CAPTA, NS-MBS) | OFAC | Partial restrictions by program |
| Consolidated Sanctions List | UN (United Nations) Security Council | Universal |
...
Indirect exposure is not a list match. OFAC VC Industry Guidance (2021) expects
