...
- `packages/sdk/deployments/31337.json` — AccessManager, L1/L2/L3, AmlHook, FeeEscrow, ComplianceTreasury, `feeToken`, `usdFeed` / `ethUsdFeed`, demo wallets A–E (Anvil #1–#5), attestor, role holders (`hookGovernor`, `complianceOfficer`, keepers), poolManager. Wallet F is a live OFAC SDN ETH address bound by the API, not this JSON.
- `apps/api/.env.local` — RPC, hook, oracle, escrow, token, feeds, `COMPLIANCE_TREASURY_ADDRESS` / `COMPLIANCE_RESERVE` / `LP_COMPENSATION_FUND`, keeper (Anvil #0), attestor (Anvil #9)
...
Local Anvil deploy binds `MockUsdFeed` for the fee token ($1) and ETH ($1000). On a live chain Deploy binds official Chainlink ETH/USD (native + WETH) and USDC/USD. `_HOOK_GOVERNOR` can retune extras via `AmlHook.setPriceFeed(token, aggregator)` (`address(0)` = ETH/USD). `_COMPLIANCE_OFFICER` proposes then confirms USD floors and floor fees (48h). Never-scored magnitude floors are USD-8 (`1_000e8` / `15_000e8`). If `lastFx` is younger than 30 minutes (`FX_HOT_TTL`) the swap does not call Chainlink; otherwise one round per token. A missing live round uses `lastFx` (max 24h). No live round and no cache within 24h fail-closes.
