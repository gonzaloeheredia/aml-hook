...
Shared ABIs (application binary interfaces), types, and deployment addresses for AML (Anti-Money Laundering) Hook.
...
`ComplianceOracle.updateScore` is AccessManaged: the API's (application programming interface) `KEEPER_PRIVATE_KEY` must be the granted oracle keeper (Anvil #0 by default on local Deploy). The attestor (Anvil #9 locally) signs `attestationHash`. The COA (Compliance Officer Agent) emits `finalScore` and `recommendedFeeBps`. The keeper only publishes that row.

Local Anvil deploy binds `MockUsdFeed` for the fee token ($1) and ETH ($1000). On a live chain Deploy binds official Chainlink ETH/USD (United States dollar) (native + WETH) and USDC/USD. `_HOOK_GOVERNOR` can retune extras via `AmlHook.setPriceFeed(token, aggregator)` (`address(0)` = ETH/USD). `_COMPLIANCE_OFFICER` proposes then confirms USD floors and floor fees (48h). Never-scored magnitude floors are USD-8 (`1_000e8` / `15_000e8`). Liquidity adds reuse those cuts via `LpPolicyLib` (Floor A / C / D; C sums adds, not swaps). If `lastFx` is younger than 30 minutes (`FX_HOT_TTL`) the swap does not call Chainlink. Otherwise the hook reads one round per token. A missing live round uses `lastFx` (max 24h). No live round and no cache within 24h fail-closes.
