...
- `packages/sdk/deployments/31337.json` — AccessManager, L1/L2/L3, AmlHook, FeeEscrow, LpCompensationVault, ComplianceTreasury, `feeToken` (MockUSDC), `wethToken` (MockWETH), `usdFeed` / `ethUsdFeed`, demo wallets A–E (Anvil #1–#5), attestor, role holders (`hookGovernor`, `complianceOfficer`, keepers), poolManager.
...
- `apps/api/.env.local` — RPC, hook, oracle, escrow, fee token, `WETH_TOKEN_ADDRESS`, feeds, `COMPLIANCE_TREASURY_ADDRESS` / `COMPLIANCE_RESERVE` / `LP_COMPENSATION_FUND` / `LP_COMPENSATION_VAULT` / `COMPENSATION_LPS`, keeper (Anvil #0), attestor (Anvil #9)
