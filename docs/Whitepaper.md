...
Mitigation D, recent-funds inflow heuristic. Before RiskPolicy.decide runs, beforeSwap reads the end-user's balance of the swap's input token through a minimal IERC20 (fungible token interface) balanceOf call and compares it to lastKnownBalance for that wallet and token. The delta as a share of the current balance is expressed in bps; if that share exceeds inflowThresholdBps (default 5000, or 50 percent) and the oracle's updatedAt is still at or before the last known balance timestamp, the hook passes hasSignificantInflow as true. RiskPolicy then applies the same FEE_OVERRIDE floor as Mitigation B. isStale and hasSignificantInflow do not accumulate into a harsher tier when both are true: the floor remains FEE_OVERRIDE unless the score already maps to REVERT. When the signal fires, the hook emits InflowHeuristicTriggered(wallet, deltaBps, timestamp) for the audit trail. afterSwap refreshes lastKnownBalance and lastKnownBalanceTimestamp with the post-swap balance so the next beforeSwap has a correct baseline. The extra balanceOf call is an intentional gas cost of this architecture, on the order of a few thousand gas for a typical ERC-20 read.

The inflow heuristic detects a generic behavioral pattern (recent funds followed by a swap attempt). It does not trace the origin of those funds. N-hop decay scoring remains the mechanism that attributes contamination through hops; the balance heuristic only narrows the exposure window until the keeper processes the transfer.

These mitigations elevate ALLOW to FEE_OVERRIDE. They leave REVERT and an existing FEE_OVERRIDE from the score bands intact. Elevations emit LatencyMitigationApplied with a reason code (SCORE_NEVER_WRITTEN, STALE_WITH_POOL_ACTIVITY, or ACTIVITY_WINDOW_CAP) and, for the inflow path, InflowHeuristicTriggered. Default deploy parameters in the local stack are stalenessThreshold = 5 minutes, activityWindow = 1 hour, maxOpsInWindow = 3, inflowThresholdBps = 5000; the owner may retune stalenessThreshold and inflowThresholdBps after deploy.
