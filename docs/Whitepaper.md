...
The same structural ambiguity appears with the Uniswap router. In a normal swap path the address the PoolManager reports to beforeSwap as the swap initiator (the router that called PoolManager.swap) is not the end user. Scoring or sanctioning that router address would be meaningless: either every swap would inherit a clean router score and bypass control, or a contaminated router score would block the entire pool. AML Hook therefore never treats the router as the compliance subject.

Subject resolution uses a two-path model. The primary path is a trusted-router registry maintained through setTrustedRouter by the AccessManager role `_HOOK_GOVERNOR` — deliberately separate from `_REGISTRY_KEEPER` and `_ORACLE_KEEPER`, so the automated scoring key cannot retune which routers the hook trusts. When the PoolManager-reported swap initiator is registered as trusted, the hook calls IMsgSender.msgSender() on that router inside a try/catch. IMsgSender is a minimal external view interface that returns the end-user address for the current swap. A non-zero return is the verified compliance subject. If hookData also encodes a non-zero address, that value is treated as a cross-check only: agreement confirms the subject; disagreement reverts with SubjectMismatch(declared, fromRouter) before any compliance layer runs. If msgSender() reverts or returns zero, the hook reverts with TrustedRouterSubjectFailed — it does not fall back to caller-supplied hookData. The trusted router is therefore the source of truth when present; a failed report fails closed.

Enablement of a trusted router is an operational off-chain decision, not a property the contract can verify on-chain. Before calling setTrustedRouter with trusted equal to true, the hook governor must confirm that the address belongs to a curated list of direct integrators — for example Uniswap Labs routers and other protocols with a known integration relationship — and that the router's msgSender() implementation was reviewed to return the real end-user address and that no third party can overwrite that value within the same transaction. The on-chain registry only records that governor attestation; the guarantee depends on that pre-call audit process.

The fallback path applies only when the initiator is not trusted. In that case the hook decodes abi.encode(endUser) from hookData. If hookData is missing, empty, or decodes to zero, beforeSwap reverts with MissingSwapSubject before Layer 1, Layer 2, or Layer 3 run. There is never a fallback that scores the router itself. Integration responsibility sits with pool operators (who register trusted routers after that review) and with routers that implement IMsgSender; untrusted or undeclared paths still require a valid hookData subject or the operation does not execute.

AML Hook resolves institutional Smart Accounts with two verification models. In controller verification, the hook checks who the owners of the Smart Account are and verifies each one individually, so that if any controller is sanctioned, the swap is blocked. In threshold verification, it is not enough for one controller to be clean, it verifies that there are enough clean controllers to meet the multisig's approval threshold. If the threshold is three of five and a sanctioned controller is not enough to approve anything on its own, the swap can proceed. In both models the address under evaluation remains the resolved end-user subject (trusted-router msgSender, or hookData on the fallback path), not the router that forwarded the call.
...
The Compliance Officer Agent (COA) analyzes the case off-chain. It has no write function on FeeEscrow. Every on-chain release or confiscation is executed exclusively by a FeeEscrow keeper (that contract's own access list — not the shared AccessManager roles used for sanctions, scores and hook governance). Before submitting a transaction, the keeper applies an off-chain sanity rule on the COA output (for example a minimum confidence threshold).

There are two COA consultations on the escrow path, and each maps to a fixed on-chain destination for the retained fee:
...
Mitigation D, recent-funds inflow heuristic. This is the mitigation that closes the causal gap Mitigations A through C leave open: a wallet that was previously published as clean (score 0, non-zero updatedAt), receives a large peer-to-peer transfer, and attempts a swap before the keeper's updateScore confirms. Mitigations A through C do not elevate that path, because the score is written, may still be inside stalenessThreshold, and the wallet has no prior pool activity. Before RiskPolicy.decide runs, beforeSwap reads the end-user's balance of the swap's input token through a minimal IERC20 (fungible token interface) balanceOf call and compares it to lastKnownBalance for that wallet and token. The delta as a share of the current balance is expressed in bps; if that share exceeds inflowThresholdBps (default 5000, or 50 percent) and the oracle's updatedAt still predates the inflow baseline, the hook passes hasSignificantInflow as true. RiskPolicy then applies a FEE_OVERRIDE floor. When no keeper-recommended feeBps is available, the designed product behavior for this path is the intermediate latency fee of 8 percent (800 bps), the same economic friction used for other oracle-latency elevations, rather than an unconditioned ALLOW at the pool's 0.30 percent base. isStale and hasSignificantInflow do not accumulate into a harsher tier when both are true: the floor remains FEE_OVERRIDE unless the score already maps to REVERT. When the signal fires, the hook emits InflowHeuristicTriggered(wallet, deltaBps, timestamp) for the audit trail. afterSwap refreshes lastKnownBalance and lastKnownBalanceTimestamp with the post-swap balance so the next beforeSwap has a correct baseline. The extra balanceOf call is an intentional gas cost of this architecture, on the order of a few thousand gas for a typical ERC-20 read. The accompanying use case develops this path end-to-end as Wallet D (section 7).
