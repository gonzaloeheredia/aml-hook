...
AmlHook              Orchestrator at swap    SanctionRegistry, ComplianceOracle,        Pool activity in afterSwap; SwapObserved;         Calculate the behavioral score
                     time (beforeSwap /      RiskPolicy; resolves end-user via           LatencyMitigationApplied; may revert            off-chain; hold the sanctions
                     afterSwap)              trusted IMsgSender router and/or           (SanctionHit, WalletBlocked,                     list itself
                                             hookData                                   MissingSwapSubject, SubjectMismatch)
-------------------- ----------------------- ------------------------------------------ -------------------------------------------------- --------------------------------
...
Subject resolution uses a two-path model. The primary path is a trusted-router registry maintained by the hook owner through setTrustedRouter, following the same administrative pattern as SanctionRegistry. When the PoolManager-reported swap initiator is registered as trusted, the hook calls IMsgSender.msgSender() on that router inside a try/catch. IMsgSender is a minimal external view interface that returns the end-user address for the current swap. A non-zero return is the verified compliance subject. If hookData also encodes a non-zero address, that value is treated as a cross-check only: agreement confirms the subject; disagreement reverts with SubjectMismatch(declared, fromRouter) before any compliance layer runs. The trusted router is therefore the source of truth when present; hookData does not override it.

Enablement of a trusted router is an operational off-chain decision, not a property the contract can verify on-chain. Before calling setTrustedRouter with trusted equal to true, the owner must confirm that the address belongs to a curated list of direct integrators — for example Uniswap Labs routers and other protocols with a known integration relationship — and that the router's msgSender() implementation was reviewed to return the real end-user address and that no third party can overwrite that value within the same transaction. The on-chain registry only records the owner's attestation; the guarantee depends on that pre-call audit process.
...
Mitigation B, stale score with recent pool activity. AmlHookLogic compares block.timestamp minus WalletRisk.updatedAt against a configurable stalenessThreshold (seconds; default 60 for high-volume institutional pools, owner-updatable via setStalenessThreshold). When the score is stale and the wallet's operationCount in the current activity window is greater than zero, RiskPolicy applies a floor: the result cannot be ALLOW even if the stored score sits in the low band. The minimum outcome is FEE_OVERRIDE, using the intermediate-tier fallback fee in basis points (bps, hundredths of a percent) when the keeper did not publish a usable recommendedFeeBps. A stale score with zero operations in the current window leaves the stored score mapping unchanged.
