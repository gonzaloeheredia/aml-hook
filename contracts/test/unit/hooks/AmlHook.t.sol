...
import {BaseHook} from "contracts/external/BaseHook.sol";
...
contract UnitAmlHookTest is Helpers {
    ...
    function test_FeeOverrideReturnsV4FeeWithFlag() external {
        vm.prank(keeper);
    ...
    function test_TwoHopProportionalFee() external {
        vm.prank(keeper);
        ...
        (,, uint24 fee) = manager.callBeforeSwap(IHooks(address(hook)), router, key, params, data);
        assertTrue(fee.isOverride());
        assertEq(fee.removeOverrideFlag(), 30_000);
    }
