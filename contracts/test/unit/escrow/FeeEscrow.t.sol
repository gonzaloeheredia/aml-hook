...
contract UnitFeeEscrowTest is HelpersCore {
    ...
    function test_SetAuditor_GrantAndRevoke() external {
        uint256 id = _deposit(1 ether);
        address auditor = makeAddr("auditor");
        vm.prank(owner);
        escrow.setAuditor(auditor, true);
        assertTrue(escrow.auditors(auditor));

        vm.prank(auditor);
        assertEq(escrow.getEscrow(id).amount, 1 ether);

        vm.prank(owner);
        escrow.setAuditor(auditor, false);
        assertFalse(escrow.auditors(auditor));
    ...
    function test_SetAuditor_RevertsOnZero() external {
        vm.prank(owner);
        vm.expectRevert(FeeEscrow.ZeroAddress.selector);
        escrow.setAuditor(address(0), true);
    }

    function test_SetAllowedFeeToken_RevokeBlocksDeposit() external {
        vm.prank(owner);
        escrow.setAllowedFeeToken(address(token), false);
        vm.prank(depositor);
        vm.expectRevert(FeeEscrow.FeeTokenNotAllowed.selector);
        escrow.deposit(walletA, address(token), ORIGIN_TX, 1 ether);
    }

    function test_SetAllowedFeeToken_RevertsOnZero() external {
        vm.prank(owner);
        vm.expectRevert(FeeEscrow.ZeroAddress.selector);
        escrow.setAllowedFeeToken(address(0), true);
    }

    function testFuzz_DepositThenDefaultReleaseCreditsLpFund(uint256 amount, bytes32 fingerprint) external {
        amount = bound(amount, 1, 1_000_000 ether);
        vm.prank(depositor);
        uint256 id = escrow.deposit(walletA, address(token), fingerprint, amount);
    ...
    function testFuzz_BlockedRecoverAlwaysGoesToReserve(uint256 amount) external {
        amount = bound(amount, 1, 1_000_000 ether);
        uint256 id = _deposit(amount);
    ...
    function testFuzz_PublicEscrowHashesWallet(address wallet, uint256 amount, bytes32 fingerprint) external {
